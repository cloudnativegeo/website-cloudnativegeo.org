...
Every single page gets a generated 1200×630 OG card as its `og:image`, rendered entirely in the Hugo build — no headless browser or Node step. Two styles: the **Masthead** card (blog + generic pages) and an inverted **Event** card (the `events` section — blue ground, white headline, `venue · time` / URL footer). The dispatcher/renderer is `layouts/partials/og/card.html` (wired in via `opengraph.html`), with `card-event.html` for events and a shared `fit.html` auto-fit; text is overlaid onto the committed chrome `assets/og/base.png` / `base-event.png`, sized via the font metrics in `data/og_metrics.json`. For non-event pages, explicit front-matter `images:` still overrides; for events, `images:` is the in-page hero and the event card is always the `og:image`. Flagship events can set `og_card_photo:` (+ `og_card_treatment: duotone`) to use a build-time Bonus-Blue duotone of a photo as the card ground. See `scripts/og/README.md` for the full pipeline.
