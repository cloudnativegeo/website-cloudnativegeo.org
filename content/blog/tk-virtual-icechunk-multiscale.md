...
// --- register numcodecs codecs used by the virtualized NOHRSC NetCDF ---
// Virtualized NetCDF stores carry `numcodecs.*`-prefixed codec names that
// zarr-layer's codec registry doesn't recognize by default.
...
// --- colormaps (carbonplan/colormaps; water + redteal, light + dark variants) ---
// Generated via `makeColormap(name, { count: 255, mode, format: 'hex' })`.
const CMAPS = {
...
// Season dim indices — verified from store metadata: season[0]='2024_2025', season[1]='2025_2026'
const PREV_YEAR_IDX = 0
const THIS_YEAR_IDX = 1

// --- single source of truth for tunable map / view config ---
const CONFIG = {
  center: [-113.7, 40.7],
  zoom: 3.70,
  bounds: [-126, 21, -66, 55],
  ...
  minZoom: 2.5,
  // zoom at which the virtualized NetCDF takes over from the native multiscales
  nativeZoomThreshold: 4.13,
  defaultView: 'diff',
  // per-view: colormap + clim + legend labels
  views: {
...
// --- diff shader: this year − previous year ---
const DIFF_FRAG = `
...
function setMode(mode) {
  ...
  map.addLayer(createLayer(mode), 'landuse_pedestrian')
...
map.on('load', async () => {
  try {
    store = await IcechunkStore.open(ICECHUNK_URL, {
      branch: 'main',
      formatVersion: 'v1',
       withRangeCoalescing,
    })

    map.addLayer(createLayer(currentMode), 'landuse_pedestrian')
  } catch (err) {
